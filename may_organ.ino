...
Author: Alvin La
Date: 05/20/2024
Description: The code reads in rotary encoder position and the three potentiometers to then change
the digipot and ESC to a specific associated frequency to play a musical note.
*/
#include <Servo.h>
...
/* button flip turn motor on right away*/
void MayOrgan(){
    if(digitalRead(MOTOR_SWITCH)){
        int pot_note_value = analogRead(POT_NOTE);
        digi_pot_value = map(pot_note_value, 335, 600, 0, 255);
        digi_pot_value = (digi_pot_value < 0) ? 0 : (digi_pot_value > 255) ? 255 : digi_pot_value;
        ...
        if(octave_value > POT_BUTTON_THRESHOLD){
            if(octave_value2 > POT_BUTTON_THRESHOLD){
                octave = 2;
            }
            else{
                octave = 1;
